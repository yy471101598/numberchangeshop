...
public class ImpObtainFabiDuihuanId {
    public void obtainFabiDuihuanId(final Activity ac,
                               final InterfaceBack back) {

        AsyncHttpClient client = new AsyncHttpClient();
//        final PersistentCookieStore myCookieStore = new PersistentCookieStore(ac);
//        client.setCookieStore(myCookieStore);
        client.get(ContansUtils.BASE_URL + "pos/FepID.ashx", new AsyncHttpResponseHandler() {
            @Override
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                try {
                    ...
                    Toast.makeText(ac,ac.getResources().getString(R.string.dhcodefalse), Toast.LENGTH_SHORT).show();
                }
            }
