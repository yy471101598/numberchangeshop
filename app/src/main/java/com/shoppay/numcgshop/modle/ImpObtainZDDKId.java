...
public class ImpObtainZDDKId {
    public void obtainZDDKId(final Activity ac,
                             final InterfaceBack back) {

        AsyncHttpClient client = new AsyncHttpClient();
//        final PersistentCookieStore myCookieStore = new PersistentCookieStore(ac);
//        client.setCookieStore(myCookieStore);
        client.get(ContansUtils.BASE_URL + "pos/CoinLoanID.ashx", new AsyncHttpResponseHandler() {
            @Override
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                try {
                    ...
                    Toast.makeText(ac, ac.getResources().getString(R.string.zddkcodefalse), Toast.LENGTH_SHORT).show();
                }
            }
